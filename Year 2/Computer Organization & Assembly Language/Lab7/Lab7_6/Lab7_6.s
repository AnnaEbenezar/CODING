...
sum_func:
    ...
    @ Load Link Register from backup 2
    LDR lr, addr_lr_bu_2
    LDR lr, [lr]        @ Mem[addr_lr_bu_2] <- LR

    BX lr

    @ addresses of Link Register back up to 2
...
main:
    ...
    @ Get num_1 from the user via keyboard
    LDR R0, addr_pattern
    LDR R1, addr_num_1
    BL scanf
    ...
    @ Get num_2 from the user via keyboard
    LDR R0, addr_pattern
    LDR R1, addr_num_2
    BL scanf
